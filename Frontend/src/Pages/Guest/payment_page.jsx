  ...
  const handleConfirmPayment = async () => {
    ...
    if (cardType !== 'Cash on Delivery') {
      ...
      if (!cleanedCardNumber || !/^\d{16}$/.test(cleanedCardNumber)) {
        ...
        // Additional validation for specific card types if needed
        if (cardType === 'American Express' && !/^\d{15}$/.test(cleanedCardNumber)) {
          newErrors.cardNumber = 'American Express card number must be 15 digits';
        }
      }
      ...
      } else if (cardType === 'American Express' && !/^\d{4}$/.test(cvv)) {
        newErrors.cvv = 'CVV for American Express must be 4 digits';
      } else if (cardType !== 'American Express' && !/^\d{3}$/.test(cvv)) {
        ...
        <PaymentForm>
          ...
          <FormField>
            ...
            <FormInput
              ...
              onChange={(e) => {
                // Remove any non-numeric characters and limit the number to 16 digits
          ...
          <FormInput
            ...
            onChange={(e) => {
              ...
              if (value.length >= 3) {
                // Insert '/' after the second digit (for MM/YY format)
